...
describe("flattenRenderStateToText", () => {
  // Removed: literal-format pinning of every render-DSL component
  // (empty/title/metric/progress/status/table/list/code/sparkline/kv/
  // timeline/callout/separator). These tests asserted exact markdown
  // strings, which couples to formatting choices rather than the
  // agent-readable contract. See docs/testing-strategy.md §5 — the §2.6
  // rewrite (Wave 2) replaces these with property-style assertions
  // (e.g. heading shape `^# .+$`, presence of label tokens). Until then
  // this surface is intentionally uncovered.
