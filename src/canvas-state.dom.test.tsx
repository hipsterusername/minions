...
describe("useCanvasHistory — dispatch + history tracking", () => {
  ...
  it("SET_NODES bypasses the history stack — neither past nor future change", () => {
    // §6.4 mutation-test follow-up: the previous version of this test
    // only checked `canUndo` after SET_NODES — but `canUndo` is true
    // both under the original `HISTORY_ACTIONS.has(action.type)` and
    // the mutated `if (true)`. The discriminator is the FUTURE stack:
    // the dispatch path's `futureRef.current = []` clears redo state
    // under the mutation. Set up a non-empty future via undo first.
    const h = mount();
    act(() => {
