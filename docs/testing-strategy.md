...
- Broad full-browser end-to-end coverage. One credential-free Playwright smoke
  journey protects clean-clone startup, project creation, session launch,
  persistence, and reload; detailed behavior remains covered at L1–L3.
...
```
┌──────────────────────────────────────────────────────────────┐
│  L5 — End-to-end (Playwright; one deterministic smoke path) │
├──────────────────────────────────────────────────────────────┤
...
**Target.** A clean-clone confidence path: create a project, launch the explicit
test-only echo harness, wait for persistence, reload, and recover the session.

**Tool.** Playwright + Chromium, serial and credential-free.

**Where.** `tests/e2e/clean-clone.smoke.spec.mjs`.

This lane intentionally stays narrow. Provider authentication, model quality,
and exhaustive UI interactions belong to provider checks or L1–L3 tests rather
than the merge-gating smoke journey.
...
| Browser smoke | **Playwright** | `pnpm test:smoke`. One serial Chromium journey using the explicit test-only echo harness. |
...
4. Extend L5 only when the clean-clone journey gains a new critical boundary;
   keep detailed behavior at L1–L3.
...
- `pnpm test:coverage`, with the diagnostic summary and report uploaded.
- `pnpm test:smoke`, after CI installs Chromium.
- `pnpm audit:prod`, failing on high-severity production dependency findings.

### Coverage

Reported, not gated. CI publishes `coverage/` and writes the overall, server,
frontend, and shared summary generated by `pnpm coverage:report`. The number we
care about is the **trend**: a PR that reduces coverage on a module that had it
is a flag worth a comment in review. High-risk contracts are judged by their
behavioral cases, not hidden by a repository-wide percentage.
