/**
 * server/worktree-merge — three-way merge with rebase fallback and strategy
 * resolution.
 *
 * Per docs/testing-strategy.md §5.2 we mock `node:child_process`. The
 * merge function makes many ordered git calls; we drive the mock in
 * lockstep. The contract we pin:
 *
 *   1. Happy path: merge succeeds → fast-forward target via update-ref
 *      → success result.
 *   2. Conflict, no strategy, default rebase=true: merge --abort → rebase
 *      succeeds → fast-forward succeeds → success.
 *   3. Conflict, no strategy, rebase fails: returns success=false with
 *      conflict file list and rebase aborted.
 *   4. Conflict with strategy="ours": manual file checkout per unresolved
 *      file → add -A → commit → fast-forward → success.
 *   5. mergeAndCleanup: on success calls removeWorktree; on failure
