...
/**
 * Repair rows written by older builds that marked a run completed without
 * carrying the report required by the public snapshot contract.
 */
export function repairInvalidCompletedWorkItemRuns(
  ...
  return db.transaction(() => {
    const rows = db.prepare(`
      SELECT session_key, work_item_id, run_kind
      FROM sessions
...
/** Seal one specific missing active primary so continuation can allocate a new immutable run. */
  ...
  return db.transaction(() => {
    const item = db.prepare(`SELECT lifecycle_revision FROM work_items
      WHERE id = ? AND current_run_key = ? AND runtime_state IN ('starting','working','waiting')
        AND outcome = 'none'`).get(workItemId, runKey) as
      { lifecycle_revision: number } | undefined;
