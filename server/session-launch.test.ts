...
describe("launchSession", () => {
  it("switches an unavailable harness without carrying its model", async () => {
    ...
    expect(result).toMatchObject({ harness: "codex", model: "gpt-5.6-sol", permissionMode: "auto", reasons: ["harness_not_ready", "model_incompatible", "permission_unsupported"] });
    expect(h.events[0]).toMatchObject({ type: "session_launch_resolved", transient: true });
    expect(h.starts[0]).toMatchObject({ harness: "codex", initialModel: "gpt-5.6-sol", permissionMode: "auto" });
  });
