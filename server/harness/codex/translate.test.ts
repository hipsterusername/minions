/**
 * Unit tests for createCodexTranslator (server/harness/codex/translate.ts).
 *
 * Table-driven: one describe per source event/item variant from the mapping
 * table in docs/codex-harness-spec.md §"Event translation". Each test
 * constructs a minimal SDK event and asserts the produced NormalizedEvent[]
 * shape is exactly correct.
 *
 * The test file is also typechecked by pnpm typecheck:server — input objects
 * must satisfy the real SDK types.
 */
...
// ── item.completed / mcp_tool_call (failed) ───────────────────────────────────
...
describe("item.completed: file_change", () => {
  ...
  it("emits only tool_result (no synthetic tool_call) when item.started was seen", () => {
    ...
    // Simulate item.started being emitted for this file_change
    tr.translate({
      type: "item.started",
...
// ── elapsedSeconds with mock clock ───────────────────────────────────────────
