...
export interface WorkItemService {
  ...
  updateCard(input: WorkItemMutationContext & { workItemId: string; expectedWorkflowRevision: number;
    title?: string; patch: Partial<KanbanCardMetadata> }): Promise<WorkItemDetailSnapshot>;
  moveCard(input: WorkItemMutationContext & { workItemId: string; expectedWorkflowRevision: number;
    columnId: string; targetIndex?: number; rank?: string }): Promise<WorkItemDetailSnapshot>;
  importKanban(input: WorkItemMutationContext & { projectId: string; projectPath: string;
    migrationKey: string; cards: KanbanImportCard[] }): Promise<WorkItemListSnapshot>;
