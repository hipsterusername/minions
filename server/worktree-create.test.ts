/**
 * server/worktree-create — git worktree lifecycle (create/remove/list/cleanup).
 *
 * Per docs/testing-strategy.md §5.2, we mock at the `node:child_process`
 * boundary and run the real `worktree-exec.ts` wrapper. The mock buffers a
 * queue of `{ args, result }` expectations the test seeds; each `exec` call
 * pulls from the head and asserts the args match.
 */
...
interface QueuedCall {
  // The args we expect to see — `null` slots match any value.
