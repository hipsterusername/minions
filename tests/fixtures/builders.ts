...
export function taskStatusUpdate(
  ...
  return {
    ...
    payload: {
      taskId,
      status,
      activeStep: status === "in_progress" ? "working" : null,
      progress: [],
...
export function taskResult(taskId: string, success = true): EdgeMessage {
  return {
...
export function makeStatusEdge(
  id: string,
  sourceNodeId: string,
  targetNodeId: string,
  overrides: Partial<GraphEdge> = {},
...
export function makeResultEdge(
  id: string,
  sourceNodeId: string,
  targetNodeId: string,
  overrides: Partial<GraphEdge> = {},
